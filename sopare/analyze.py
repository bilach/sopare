...
class Analyze:
    ...
    def do_analysis(self, results, data, rawbuf):
        ...
        readable_results, self.debug_info = self.stm.get_results(
            readable_results, self.debug_info
        )
    ...
    def framing(self, results, data_length):
        ...
        for id in results:
            ...
            for i, row in enumerate(results[id]):
                ...
                row_result = (
                    sum(row[0 : len(row)]) / self.dict_analysis[id]["min_tokens"]
                )
        ...
        for el in sorted_arr:
            if el[1] not in framing[el[2]] and (
                self.cfg.getintoption("compare", "MAX_WORD_START_RESULTS") == 0
                or len(framing[el[2]])
                < self.cfg.getintoption("compare", "MAX_WORD_START_RESULTS")
            ):
                framing[el[2]].append(el[1])
        return framing

    def row_validation(self, row, _):
        if row[0] == 0 or len(row) <= self.cfg.getintoption(
            "compare", "MIN_START_TOKENS"
        ):
            return [0] * len(row)
        return row
    ...
    def deep_search(self, framing, data):
        ...
        for _id in framing:
            for startpos in framing[_id]:
                word_sim = self.deep_inspection(_id, startpos, data)
                if len(word_sim) > 0:
        ...
        for match in framing_match:
            ...
            nobm = 1
            if (
                self.cfg.hasoption("compare", "NUMBER_OF_BEST_MATCHES")
                and self.cfg.getintoption("compare", "NUMBER_OF_BEST_MATCHES") > 0
            ):
        ...
        for i, best in enumerate(sorted_best_match):
            if (
                best[0] >= self.cfg.getfloatoption("compare", "MIN_CROSS_SIMILARITY")
                and best[1] <= self.cfg.getfloatoption("compare", "MIN_LEFT_DISTANCE")
                and best[2] <= self.cfg.getfloatoption("compare", "MIN_RIGHT_DISTANCE")
            ):
                for x in range(best[3], best[3] + best[4]):
                    if match_results[x] == "":
                        match_results[x] = best[5]
            if self.cfg.getintoption(
                "compare", "MAX_TOP_RESULTS"
            ) > 0 and i > self.cfg.getintoption("compare", "MAX_TOP_RESULTS"):
                break
        self.debug_info += str(match_results).join(["match_results: ", "\n\n"])
        return match_results

    def token_sim(self, characteristic, dcharacteristic):
        sim_norm = self.util.similarity(
            characteristic["norm"], dcharacteristic["norm"]
        ...
        sim_df = self.util.single_similarity(
            characteristic["df"], dcharacteristic["df"]
        ) * self.cfg.getfloatoption("compare", "SIMILARITY_DOMINANT_FREQUENCY")
        sim = sim_norm + sim_token_peaks + sim_df
        sl, sr = self.util.manhatten_distance(
            characteristic["norm"], dcharacteristic["norm"]
        )
        return sim, sl, sr

    def deep_inspection(self, _id, startpos, data):
        word_sim = []
        for dict_entries in self.learned_dict["dict"]:
            if _id == dict_entries["id"]:
                dict_characteristic = dict_entries["characteristic"]
                token_sim = [0, 0, 0, startpos, 0, _id]
                c = 0
                for i, dcharacteristic in enumerate(dict_characteristic):
                    if startpos + i < len(data):
                        ...
                        if "shift" in characteristic:
                            ssim, ssl, ssr = self.token_sim(
                                characteristic["shift"], dcharacteristic
                            )
                            if ssim > sim:
                                ssim = sim
                            if ssr < sr:
                ...
                if c > 0:
                    ...
                    if (
                        token_sim[0] > 1.0
                        and c >= self.cfg.getintoption("compare", "MIN_START_TOKENS")
                        and c >= self.dict_analysis[_id]["min_tokens"]
                    ):
                        self.logger.warning(
                            "Your calculation basis seems to be wrong as we get results > 1.0!"
                        )
                    token_sim[1] = token_sim[1] / c
                    token_sim[2] = token_sim[2] / c
                    token_sim[4] = int(c)
                if (
                    self.cfg.getbool("compare", "STRICT_LENGTH_CHECK") == False
                    and c >= self.cfg.getintoption("compare", "MIN_START_TOKENS")
                ) or c >= self.dict_analysis[_id]["min_tokens"] - self.cfg.getintoption(
                    "compare", "STRICT_LENGTH_UNDERMINING"
                ):
    ...
    def get_match(self, framing):
        ...
        for x in range(0, len(framing)):
            if x > 0 and framing[x] != framing[x - 1]:
                match_results = self.validate_match_result(
                    framing[s:x], s, x, match_results
                )
                s = x
                if x == len(framing) - 1:
                    match_results = self.validate_match_result(
        ...
        if framing.count("") > len(framing) * self.cfg.getfloatoption(
            "compare", "FILL_RESULT_PERCENTAGE"
        ):
            if self.debug:
                self.debug_info += (
                    "Results contain too many empty tokens. "
                    + str(framing.count(""))
                    + " / "
                    + str(len(framing))
                    + " Eliminating results"
    ...
    def validate_match_result(self, result, start, end, match_results):
        if len(result) == 0 or result[0] == "":
            return match_results
        if self.cfg.getbool("compare", "STRICT_LENGTH_CHECK") == True and (
            len(result)
            < self.dict_analysis[result[0]]["min_tokens"]
            - self.cfg.getintoption("compare", "STRICT_LENGTH_UNDERMINING")
            or len(result) > self.dict_analysis[result[0]]["max_tokens"]
        ):
            if self.debug:
                self.debug_info += (
                    "STRICT_LENGTH_CHECK failed for "
                    + result[0]
                    + ": "
                    + str(self.dict_analysis[result[0]]["min_tokens"])
                    + " > "
                    + str(len(result))
                    + " < "
                    + str(self.dict_analysis[result[0]]["max_tokens"])
                    + "\n"
                )
