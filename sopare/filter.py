...
class filtering:
    ...
    def get_chunked_norm(self, nfft):
        ...
        for x in range(0, nfft.size, i):
            if self.cfg.hasoption(
                    "characteristic", "START_PROGRESSIVE_FACTOR"
            ) and x >= self.cfg.getfloatoption(
                "characteristic", "START_PROGRESSIVE_FACTOR"
            ):
    ...
    def n_shift(self, data):
        ...
        if self.data_shift_counter == 0:
            self.data_shift = [
                v for v in range(0, int(self.cfg.getintoption("stream", "CHUNKS") / 2))
            ]
    ...
    def filter(self, data, meta):
        ...
        if (
                not self.first
                or not self.cfg.getbool("characteristic", "HANNING")
                or len(data) < self.cfg.getintoption("stream", "CHUNKS")
        ):
        ...
        if (
                (shift_fft is not None)
                and self.cfg.hasoption("experimental", "FFT_SHIFT")
                and self.cfg.getbool("experimental", "FFT_SHIFT") == True
        ):
            ...
            shift_nfft = fft[
                         self.cfg.getintoption(
                             "characteristic", "LOW_FREQ"
                         ): self.cfg.getintoption("characteristic", "HIGH_FREQ")
                         ]
            ...
            shift_characteristic = self.characteristic.getcharacteristic(
                shift_fft, shift_normalized, meta
            )
