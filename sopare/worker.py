...
class worker(multiprocessing.Process):
    def __init__(self, cfg, queue):
        ...
        self.compare = sopare.comparator.compare(
            self.cfg.getbool("cmdlopt", "debug"), self.util
        )
        self.running = True
        self.counter = 0
        self.plot_counter = 0
    ...
    def save_wave_buf(self):
        self.util.savefilteredwave(
            "filtered_results" + str(self.reset_counter), self.rawbuf
        )

    def remove_silence(self, m):
        silence = (
            (
                self.cfg.getintoption("stream", "LONG_SILENCE")
                * self.cfg.getintoption("stream", "CHUNK")
            )
            / 4096
        ) - 4  # TODO: Find auto value or make configurable
        if silence < 0:
            silence = 0
        ...
        if len(self.raw_character) > 0:
            for x in range(
                len(self.raw_character) - 1, len(self.raw_character) - silence, -1
            ):
    ...
    def run(self):
        ...
        while self.running:
            ...
            if obj["action"] == "data":
                raw_token = obj["token"]
                if (
                    self.cfg.getbool("cmdlopt", "wave") == True or True
                ...
                if characteristic is not None:
                    ...
                    if self.cfg.getbool("cmdlopt", "wave"):
                        self.util.savefilteredwave(
                            "token" + str(self.counter) + self.uid, raw_token
                        )
                    if self.cfg.getbool("cmdlopt", "plot") and self.plot_counter < 6:
                        self.visual.create_sample(
                            characteristic["norm"],
                            "norm" + str(self.plot_counter) + ".png",
                        )
                        self.visual.create_sample(
                            fft, "fft" + str(self.plot_counter) + ".png"
                        )
                    self.plot_counter += 1
                self.counter += 1
            elif (
                obj["action"] == "reset"
                and self.cfg.getoption("cmdlopt", "dict") is None
            ):
                self.reset()
                ...
                for m in meta:
                    if m["token"] == "start analysis":
                        ...
                        if self.cfg.getoption("cmdlopt", "dict") is None:
                            self.analyze.do_analysis(
                                self.compare.get_results(), self.character, self.rawbuf
                            )
                        else:
                            self.util.store_raw_dict_entry(
                                self.cfg.getoption("cmdlopt", "dict"),
                                self.raw_character,
                            )
                        self.reset()
