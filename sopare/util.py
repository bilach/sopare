...
class util:
    ...
    def showdictentry(self, sid):
        ...
        ids = []
        for dict_entries in json_data["dict"]:
            if (dict_entries["id"] == sid or sid == "*") and dict_entries[
                "id"
            ] not in ids:
                print((dict_entries["id"] + " - " + dict_entries["uuid"]))
    ...
    def compile_analysis(json_data):
        ...
        for dict_entries in json_data["dict"]:
            ...
            if (
                l < analysis[dict_entries["id"]]["min_tokens"]
                or analysis[dict_entries["id"]]["min_tokens"] == 0
            ):
        ...
        for filename in os.listdir("dict/"):
            if filename.endswith(".raw"):
                ...
                with open("dict/" + filename) as raw_json_file:
                    json_obj = json.load(
                        raw_json_file, object_hook=sopare.numpyjsonencoder.numpyjsonhook
                    )
                    for raw_obj in json_obj["characteristic"]:
                        meta = raw_obj["meta"]
                        fft = raw_obj["fft"]
                        norm = raw_obj["norm"]
                        characteristic = self.characteristic.getcharacteristic(
                            fft, norm, meta
                        )
                        if characteristic is not None:
                            for m in meta:
